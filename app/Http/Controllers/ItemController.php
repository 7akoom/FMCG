...
class ItemController extends Controller
{
    public function index(Request $request)
    {
        ...
        ->join("{$markName}","{$itemName}.markref", '=', "{$markName}.logicalref")
        ->select("{$itemName}.logicalref AS id","{$itemName}.code","{$itemName}.name as arabic_name",
        "{$itemName}.stgrpcode as group","{$itemName}.specode as brand","{$itemName}.name3 as english_name","{$markName}.code as brand_name",
        "{$markName}.descr as brand_logo", "{$itemName}.name4 as turkish_name","{$priceName}.price","{$unitName}.code AS arabic_unit",
        "{$unitName}.name AS turkish_unit","{$weightName}.grossweight")
        ->where(["{$itemName}.classtype" => $type,"{$markName}.specode" => $type,"{$priceName}.clientcode" => "", "{$priceName}.active" => "0","{$weightName}.linenr" => '1'])
        ->get();
